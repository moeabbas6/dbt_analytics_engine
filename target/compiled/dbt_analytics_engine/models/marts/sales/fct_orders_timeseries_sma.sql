...
WITH
  ...
  ,simple_moving_averages AS (
    SELECT date
          ...
          ,AVG(sales) OVER (ORDER BY date ROWS BETWEEN 250 PRECEDING AND CURRENT ROW) AS sales_sma_250
  ...
  ,standard_deviations AS (
    SELECT *
           ...
           ,STDDEV(sales) OVER (ORDER BY date ROWS BETWEEN 250 PRECEDING AND CURRENT ROW) AS sales_stddev_250
  ...
  ,bollinger_bands AS (
    SELECT *
           ...
           ,sales_sma_250 + (2 * sales_stddev_250) AS sales_sma_upper_250
           ,sales_sma_250 - (2 * sales_stddev_250) AS sales_sma_lower_250
