    ...
    create or replace table `moes-dbt-layer`.`sales`.`fct_orders_timeseries_x`
...
WITH
  ...
  ,simple_moving_averages AS (
    SELECT *
      ...
      ,AVG(sales_forecasted_trend) OVER (
        ORDER BY date ROWS BETWEEN 60 PRECEDING AND CURRENT ROW
      ) AS sales_sma_60
      ,AVG(sales_forecasted_trend) OVER (
        ORDER BY date ROWS BETWEEN 120 PRECEDING AND CURRENT ROW
      ) AS sales_sma_120
